...
namespace Commercial_Controller.Tests
{
    ...
    public class UnitTest1
    {
        ...
        public void AssertElevatorsPosition(Column chosenColumn, int[] expectedFinalPositions) {
            ...
            {
                Assert.AreEqual(chosenColumn.elevatorsList[i]._currentFloor, expectedFinalPositions[i], "Elevator " + chosenColumn.elevatorsList[i]._ID + " didn't finish at the correct floor, expected " + expectedFinalPositions[i] + ", got " + chosenColumn.elevatorsList[0]._currentFloor);
            }
        }
        ...
        public void TestScenario1()
        {
            ...
            Assert.AreEqual(chosenColumn._ID, expectedColumn._ID, "Wrong column selected, expected Column " + expectedColumn._ID + ", got Column " + chosenColumn._ID);

            Assert.AreEqual(chosenElevator._ID, expectedElevator._ID, "Wrong elevator selected, expected Elevator " + expectedElevator._ID + ", got Elevator " + expectedElevator._ID);

            Assert.IsTrue(chosenElevator.completedRequestsList.Contains(userPosition), "No elevator was sent to pick up the user");

            Assert.IsTrue(chosenElevator.completedRequestsList.Contains(userPosition) && chosenElevator._currentFloor == destination, "The user didn't reach its destination");
        ...
        public void TestScenario2()
        {
            ...
            Assert.AreEqual(chosenElevator._ID, expectedElevator._ID, "Wrong elevator selected, expected Elevator " + expectedElevator._ID + ", got Elevator " + chosenElevator._ID);
